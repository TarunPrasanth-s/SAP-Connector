...
// MAIN COMPONENT
// ══════════════════════════════════════════════════════════════════════════
export default function ProxySystemSetup() {
  ...
  // Generate a proxy SCIM URL based on system name
    ...
    <div className="min-h-screen bg-gray-50 flex items-center justify-center p-6">
      <div className="w-full max-w-xl space-y-3">
        ...
        <div className="mb-6 text-center">
          <h1 className="text-base font-bold text-gray-900">Proxy System Configuration</h1>
        </div>

        {/* What is a Proxy System */}
        <Notice type="info">
          A proxy system exposes your backend as a <strong>SCIM 2.0 endpoint</strong>, allowing external identity
          managers (e.g. SAP Identity Management) to read and write users without a direct connection.
        </Notice>

        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm divide-y divide-gray-100">
          {/* ① Back-end Type */}
          <div className="p-6">
            <SectionHeader title="Back-end Type" description="Select the type of SAP system to expose as a proxy." />
          ...
          {showAutoSetup && (
            <div className="p-6">
              <SectionHeader
                ...
                description="Download and run the batch script. It installs JDK, extracts Cloud Connector, and starts it fully automatically."
              />
          ...
          {/* ⑦ Proxy Connection Details (On-Premises) */}
          {showMapping && (
            <div className="p-6">
              <SectionHeader
                title="Proxy Connection Details"
                description="Define the virtual-to-internal mapping. This system will be exposed as a SCIM 2.0 endpoint."
              />
              <div className="grid grid-cols-2 gap-4">
                ...
                <Field
                  ...
                  hint="Actual hostname in your network"
                />
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="col-span-2">
                  <Field
                    label="URL"
                    required
                    placeholder="https://<account>.workzone.ondemand.com"
                    hint="Your SAP cloud system endpoint URL"
                  />
                </div>
                <Dropdown
                  label="Type"
                ...
                />
                <Dropdown label="Proxy Type" required options={["Internet", "OnPremise"]} />
                <Dropdown
          ...
          {/* ⑧ Proxy System Name + Generated SCIM URL */}
          {showSave && (
            <div className="p-6">
              <SectionHeader
                title="Proxy System Identity"
                description="Give this proxy a unique name. A SCIM 2.0 endpoint URL will be generated for use by external identity managers."
              />
              <div className="space-y-4">
                <div>
                  <Label required>Proxy System Name</Label>
                  <input
                    type="text"
                    placeholder="e.g. SAP-ABAP-Proxy"
                    value={systemName}
                    onChange={(e) => setSystemName(e.target.value)}
                    className="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm bg-white focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-50 transition-all hover:border-gray-300 placeholder:text-gray-300 text-gray-800"
                  />
                  <p className="text-xs text-gray-400 mt-1">Unique identifier for this proxy system</p>
                </div>
                <div>
                  <Label>Generated SCIM 2.0 Proxy URL</Label>
                  <div className="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-xs bg-gray-50 text-gray-500 font-mono break-all">
                    {proxyUrl}
                  </div>
                  <p className="text-xs text-gray-400 mt-1">
                    Share this URL with your external identity manager (e.g. SAP Identity Management)
                  </p>
                </div>
              </div>
            </div>
